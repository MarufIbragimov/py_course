...
# conn = psycopg2.connect(dbname="python_db", host="localhost", user="postgres", password="postgres", port=5432)
conn = set_connection()
...
def get_all_tasks(user_id, is_deleted):
    sql_query = f"""
        SELECT id, title, description, deadline, priority, is_done
        FROM tasks 
        WHERE 
            deleted_at IS {'NOT' if is_deleted else ''} NULL AND user_id = %s 
        ORDER BY id;
    """
